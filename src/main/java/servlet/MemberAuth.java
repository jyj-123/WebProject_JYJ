...
public class MemberAuth extends HttpServlet {
	 ...
	 * 1번만 호출되는 수명주기 메서드인 init()에서 DB연결 처리를 한다.*/
	@Override
	public void init() throws ServletException {
		 ...
		 * 매개변수로 내장객체를 전달할 수 없다. 따라서 각 내장객체를 얻어올 수 
		 * 있는 메서드가 존재한다.*/
	...
	/*
	 *service()는 서블릿의 수명주기 메서드에서 get/post 방식 모두를
	 *처리할 수 있다. */
	...
	protected void service(HttpServletRequest req, HttpServletResponse resp) 
		...
		//인증 후 회원이름을 통해 성공여부를 판단한다.
		String memberName = memberDTO.getName();
		if (memberName != null) {
			/*
			 * DTO에 회원이름이 저장되어 있다면 인증에 성곤한것으로 판단한다.*/
			req.setAttribute("authMessage", memberName + " 회원님 방가방가^^*");
		}
		else {
			/*
			 * 인증에 실패했다면 서블릿 초기화 파라미터와 비교해서 관리자인지 확인한다.
			 * 만약 관리자도 아니라면 비회원으로 판단한다.*/
