...
public class WriteController extends HttpServlet{
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) 
		...
		if(session.getAttribute("UserId")==null) {
			JSFunction.alertLocation(resp, "로그인 후 이용해주세요.",
					"../Session/LoginForm.jsp");
			//Java코드가 더이상 실행되지 않도록 차단
			return;
		}
		
		//로그인이 완료된 상태라면 쓰기페이지를 포워드한다.
		req.getRequestDispatcher("/Board2/Write2.jsp").forward(req, resp);
	}
	...
	protected void doPost(HttpServletRequest req, 
		...
		if (result == 1) { //글쓰기 성공
			//게시판 목록으로 이동
			resp.sendRedirect("../board2/listPage2.do");
		}
		else { // 글쓰기 실패
			//글쓰기 페이지로 다시 돌아간다.
			JSFunction.alertLocation(resp, "글쓰기에 실패했습니다.", "../board2/write2.do");
		}
	}
}
