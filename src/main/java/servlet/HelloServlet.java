...
/*
 ...
 2.클라이언트의 요청을 받아 처리하기 위한 doGet() / doPost() 메서드를 오버라이딩한다.
...
public class HelloServlet extends HttpServlet {
	...
	/*
	 ...
	 추가하지 않아도 실행에는 영향을 미치지 않는다. */
	private static final long serialVersionUID = 1L;
	
	/*
	 get방식의 요청을 처리하기 위한 doGet()메서드 오버라이딩.
	 만약 요청을 처리할 수 있는 메서드가 없으면 405에러가 발생하게된다.
	 */
	@Override
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) 
		...
		req.getRequestDispatcher("/Servlet/HelloServlet.jsp")
			.forward(req, resp);
		/* request 영역은 포워드 된 페이지까지 공유되므로 서블릿에서
